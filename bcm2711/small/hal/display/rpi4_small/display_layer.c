...
static int32_t CreateLayer(uint32_t devId, const LayerInfo *layerInfo, uint32_t *layerId)
{
    ...
    ////////////////
    priv->fd = open("/dev/dri/card0", O_RDWR | O_CLOEXEC);

    priv->res = drmModeGetResources(priv->fd);
	priv->crtc_id = priv->res->crtcs[0];
	priv->conn_id = priv->res->connectors[0];

	priv->conn = drmModeGetConnector(priv->fd, priv->conn_id);
	// priv->width = conn->modes[0].hdisplay;
	// priv->height = conn->modes[0].vdisplay;
    HDF_LOGE("%s: width:%d height:%d ", __func__, priv->conn->modes[0].hdisplay, priv->conn->modes[0].vdisplay);
    ...
    create.width = layerInfo->width;
	create.height = layerInfo->height;
	create.bpp = 32;
	drmIoctl(priv->fd, DRM_IOCTL_MODE_CREATE_DUMB, &create);
    priv->pitch = create.pitch;
	priv->fbSize = create.size;
    priv->handle = create.handle;
    drmModeAddFB(priv->fd, priv->width, priv->height, 24, 32, priv->pitch,
			   priv->handle, &priv->fb_id);

    map.handle = create.handle;
    drmIoctl(priv->fd, DRM_IOCTL_MODE_MAP_DUMB, &map);

    priv->fbAddr = mmap(0, priv->fbSize, PROT_READ | PROT_WRITE, MAP_SHARED, priv->fd, map.offset);
    ////////////////
    if (priv->fbAddr == NULL) {
...
static int32_t CloseLayer(uint32_t devId, uint32_t layerId)
{
    ...
    if (priv->fd >= 0) {
	    ...
	    destroy.handle = priv->handle;
	    drmIoctl(priv->fd, DRM_IOCTL_MODE_DESTROY_DUMB, &destroy);
        drmModeFreeConnector(priv->conn);
...
static int32_t Flush(uint32_t devId, uint32_t layerId, LayerBuffer *buffer)
{
    ...
    ////////////////
    drmModeSetCrtc(priv->fd, priv->crtc_id, priv->fb_id, 0, 0, &priv->conn_id, 1, &priv->conn->modes[0]);
    ///////////////
    UnlockLayerMgr();
    return DISPLAY_SUCCESS;
}
